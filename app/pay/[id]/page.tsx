...
export default function PayDetailPage() {
  ...
  // Use wagmi hooks for transaction - FIXED: using useSendTransaction for native SEI transfer
  ...
  const onPay = async () => {
    ...
    try {
      ...
      // FIXED: Send native SEI transaction using sendTransaction
      sendTransaction({
        to: to as `0x${string}`,
  ...
  const fallback = (
    `<div style="display:flex;align-items:center;justify-content:center;min-height:100vh;background:linear-gradient(135deg,#0f0f23,#1a1a2e,#16213e);color:white;">
      <div style="background:rgba(255,255,255,0.05);backdrop-filter:blur(20px);padding:32px 36px;border-radius:24px;max-width:600px;width:94%;border:1px solid rgba(255,255,255,0.1);box-shadow:0 25px 50px rgba(0,0,0,0.3);">
        ...
        <div style="opacity:0.9;font-size:18px;text-align:center;margin-bottom:8px;">💳 ${amount} SEI → ${to}</div>
        <div style="margin-top:20px;text-align:center;opacity:0.7;font-size:12px;">🔒 Secure Payment with Sei Network</div>
        <div style="margin-top:10px;text-align:center;opacity:0.6;font-size:11px;">ℹ️ Memo functionality not supported on Sei EVM</div>
      </div>
    </div>`
  );
    ...
    <main className="relative min-h-screen">
      ...
      <div className="fixed left-1/2 transform -translate-x-1/2 bottom-16 z-50 mt-10">
        <div className="rounded-2xl shadow-2xl border border-white/30 bg-black/20 backdrop-blur-xl px-6 py-4 flex items-center gap-4">
          ...
          >
            {loading ? "Preparing..." : isPending ? "Confirming..." : isConfirming ? "Processing..." : "Pay"}
          </button>
        </div>
        {error && (
  ...
  <div className="relative bg-gradient-to-br from-green-900/40 to-emerald-900/30 border border-green-400/40 rounded-2xl p-6 backdrop-blur-xl shadow-2xl">
    ...
    <div className="flex items-center gap-4 mb-4">
      <div className="relative">
        <div className="w-12 h-12 bg-gradient-to-br from-green-400 to-green-600 rounded-full flex items-center justify-center shadow-lg animate-bounce-gentle">
          <svg className="w-6 h-6 text-white animate-check-draw" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex-1">
        <div className="text-xl font-bold text-green-100 flex items-center gap-2 mb-1">
          <span className="animate-text-glow">Payment Successful!</span>
    ...
    <a 
      href={`https://testnet.seistream.app/transactions/${hash}`}
      target="_blank"
    ...
    >
      <div className="flex items-center gap-2">
        ...
        <span className="text-sm">View on Sei Explorer</span>
      </div>
      
      {/* External link icon with animation */}
      <svg className="w-4 h-4 group-hover/link:translate-x-1 group-hover/link:-translate-y-1 transition-transform duration-300" 
           fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" />
      </svg>
      
      {/* Hover glow effect */}
      <div className="absolute inset-0 bg-gradient-to-r from-green-500/0 via-green-400/20 to-green-500/0 rounded-xl opacity-0 
                      group-hover/link:opacity-100 transition-opacity duration-300 -z-10 blur-sm"></div>
    </a>
    ...
    <div className="absolute inset-0 rounded-2xl overflow-hidden pointer-events-none">
      <div className="absolute inset-0 bg-gradient-to-r from-transparent via-green-400/10 to-transparent 
                      animate-wave -translate-x-full"></div>
    </div>
  </div>
</div>
